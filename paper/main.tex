...
All data derive from two public sources---Google Patents Public Data (\texttt{patents-\allowbreak public-\allowbreak data.\allowbreak patents.\allowbreak publications}) on BigQuery and the SEC EDGAR XBRL \texttt{companyfacts} API---and all code, queries, the firm registry, the merger-reattribution table, and the pre-registration deviation log are available in the replication repository [Repository: \url{https://github.com/anilberkekarel/quality-engine-v2} --- to be made public upon submission]. The universe definition, assignee-matching rules and exclusions, predecessor-CIK merges, and the QCOM amendment are recorded with timestamps preceding the predictive analysis.
